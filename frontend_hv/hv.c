/********************************************************************\
  ...
  $Id: hv.c 3898 2007-09-11 15:04:12Z ritt@PSI.CH $
...
INT hv_init(EQUIPMENT * pequipment)
{
   ...
   // We get rid of all YBOS stuff ajc
...
//      hv_info->format = FORMAT_YBOS;
   ...
   //printf("about to initialize device drivers\n\n");
   ...
   for (i = 0; pequipment->driver[i].name[0]; i++) {
      ...
      //printf("added things ok\n");

      status = device_driver(&pequipment->driver[i], CMD_INIT, hKey);
      //printf("status for init%i, want %i\n",status,FE_SUCCESS);
      if (status != FE_SUCCESS) {
         free_mem(hv_info);
         return status;
      }
   }
   //printf("initialized device drivers\n\n");
   ...
   //printf("create/read settings\n\n");

   /*---- Create/Read settings ----*/

   //printf("about to go into relevant section\n\n");
   /* Names */
   for (i = 0; i < hv_info->num_channels; i++)
   ...
   //printf("about to create/read variables\n");
   ...
   //printf("holy crap i'm past an if statement\n");
   ...
   //printf("about to open hotlink - how hot is that???\n");
   ...
   db_open_record(hDB, hv_info->hKeyDemand, hv_info->demand,
                  ...
                  pequipment);
   //printf("read some channels from it's hotness\n");
   ...
   //printf("set some db data of Great Justice\n");
   db_set_data(hDB, hv_info->hKeyCurrent, hv_info->current,
               sizeof(float) * hv_info->num_channels, hv_info->num_channels,
               TID_FLOAT);
   pequipment->odb_out++;
...
//  YBOS is deprecated--remove commands
//
